...
type_binomial_trial100 = read.csv('100_type_binomial_trial_500.csv')
type_poisson_trial100 = read.csv('100_type_poisson_trial_500.csv')
type_ZIP_trial100 = read.csv('100_type_ZIP_trial_500.csv')
type_binomial_trial300 = read.csv('300_type_binomial_trial_500.csv')
type_poisson_trial300 = read.csv('300_type_poisson_trial_500.csv')
type_ZIP_trial300 = read.csv('300_type_ZIP_trial_500.csv')
type_binomial_trial500 = read.csv('500_type_binomial_trial_500.csv')
type_poisson_trial500 = read.csv('500_type_poisson_trial_500.csv')
type_ZIP_trial500 = read.csv('500_type_ZIP_trial_500.csv')
type_ZIP_trial1000 = read.csv('1000_type_ZIP_trial_500.csv')
type_binomial_trial1000 = read.csv('1000_type_binomial_trial_500.csv')
type_poisson_trial1000 = read.csv('1000_type_poisson_trial_500.csv')
type_binomial_trial3000 = read.csv('3000_type_binomial_trial_500.csv')
type_poisson_trial3000 = read.csv('3000_type_poisson_trial_500.csv')
type_ZIP_trial3000 = read.csv('3000_type_ZIP_trial_500.csv')

#----- Binomial
make_result = function(dataframe, truevalue, sample){
  
  dataframe = dataframe[complete.cases(dataframe),]
  ind = which(dataframe$zipest_IK!= Inf) 
  ...
  abs_local = round(mean(abs(dataframe$zipest-truevalue)/abs(truevalue)), 3)
  abs_local_IK = round(mean(abs(dataframe$zipest_IK[ind]-truevalue)/abs(truevalue)), 3)
  abs_lin = round(mean(abs(dataframe$linest - truevalue)/abs(truevalue)), 3)
...
make_graph = function(data){
  mse = data %>% gather(model, mse, contains("mse")) %>% mutate(.data = ., model = str_sub(string = model, start = -30, end = -4)) %>% 
    ...
    geom_point(mapping = aes(x = factor(N), y=mse, color=model, group=model, shape=model), size=3) +theme_bw()+
    theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), 
          ...
          legend.title = element_blank(), legend.text = element_text(size=18), legend.position='bottom') + scale_color_brewer(palette= "Set1")+ xlab('Sample size') + ylab('MSE')
  ...
  arb = data %>% gather(model, arb, contains("arb")) %>%  mutate(.data = ., model = str_sub(string = model, start = -30, end = -4)) %>% 
    ...
    geom_point(mapping = aes(x = factor(N), y=arb, color=model, group=model, shape=model), size=3) +theme_bw()+
    theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), 
          panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(), 
          #axis.ticks.x=element_blank(), 
          #axis.ticks.y=element_blank(), 
          ...
          axis.title.y = element_text(size=13),
          plot.title = element_text(face = "bold", size = 15, hjust = 0.5),
          legend.title = element_blank(), legend.text = element_text(size=18), legend.position='bottom') + scale_color_brewer(palette= "Set1")+ xlab('Sample size') + ylab('ARB')
  ...
  coverage = data %>% gather(model, coverage, contains("coverage")) %>%  mutate(.data = ., model = str_sub(string = model, start = -35, end = -10)) %>% 
    ...
    geom_point(mapping = aes(x = factor(N), y=coverage, color=model, group=model, shape=model), size=3) +theme_bw()+
    theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), 
          panel.grid.major.y = element_blank(), panel.grid.minor.y = element_blank(), 
          #axis.ticks.x=element_blank(), 
          #axis.ticks.y=element_blank(), 
          ...
          axis.title.y = element_text(size=13),
          plot.title = element_text(face = "bold", size = 15, hjust = 0.5),
          legend.title = element_blank(), legend.text = element_text(size=18), legend.position='bottom') + ylim(0, 1) +  scale_color_brewer(palette= "Set1")+ geom_hline(yintercept=0.95, linetype=2) + xlab('Sample size') + ylab('Coverage')
...
bernoulli_graph = make_graph(binomial)
poi_graph = make_graph(poi)
zip_graph = make_graph(zip)

pdf("binomial.pdf", height=6, width=15)
bernoulli_graph
dev.off()
pdf("poisson.pdf", height=6, width=15)
poi_graph
dev.off()
pdf("zip.pdf", height=6, width=15)
zip_graph
dev.off()
