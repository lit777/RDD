# Code for Model Mis-specification
library(tidyverse)
library(locfit)
library(locpol)
library(pscl)
  ...
  if (type=='ZIP'){
    ...
    for (ii in 1:trial){
      ...
      for (j in 1:length(bandwidth)){
        ...
        result_bd_zip=tryCatch(cv_rd_srd(data=data, cutoff=cutoff, q=q, bw=bandwidth[j], type='poisson')[[2]], error=function(x) return(-Inf))
...
make_csv = function(lambda, pi, cutoff, q, range, n_case, trial, type='ZIP', truediff){
    ...
    name = paste0(n_case, '_', 'type','_', type, '_', 'trial','_',trial, '_mis.csv')
...
lambdaft=function(x) {return(loglinear(-0.4*x-0.1 + (x>=0)*1.2+0.2*x*(x>=0)))}
pift = function(x) {return(logistic(0.3*x-0.5-(x>=0)*1.8))}
...
# Vary n_case (100, 300, 500, 1000, 3000)
make_csv(lambda = lambdaft, pi = pift, cutoff =0, q=0.5, range=c(-3, 3), n_case=100, trial = 500, type='ZIP', truediff = truediff_zip)
