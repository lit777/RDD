# Code for the main simulaton studies
library(tidyverse)
library(locfit)
library(locpol)
library(pscl)
...
make_csv = function(lambda, pi, cutoff, q, range, n_case, trial, type='ZIP', truediff){
    ...
    name = paste0(n_case, '_', 'type','_', type, '_', 'trial','_',trial, '.csv')
...
lambdaft=function(x) {return(loglinear(-0.4*x+1.1 + (x>=0)*1.2+0.2*x*(x>=0)))}
...
make_csv(lambda = lambdaft, pi = pift, cutoff =0, q=0.5, range=c(-3, 3), n_case=100, trial = 500, type='poisson', truediff = truediff_poi)
